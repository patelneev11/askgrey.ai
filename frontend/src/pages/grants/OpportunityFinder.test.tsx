...
function opportunity(
  overrides: Partial<GrantOpportunity> = {},
): GrantOpportunity {
  return {
    source: "grants_gov",
...
describe("opportunity search", () => {
  ...
  it("searches with the filters the user set and renders the response", async () => {
    ...
    expect(
      await screen.findByText(
        "SBIR (R43/R44) — metabolic disease therapeutics",
      ),
    ).toBeInTheDocument();
  ...
  it("names the provider that failed rather than quietly dropping it", async () => {
    ...
    expect(
      await screen.findByText(/SBIR\.gov unavailable/),
    ).toBeInTheDocument();
    // The reason has to be readable in full, so it sits outside the non-wrapping pill.
    expect(
      screen.getByText(/SBIR\.gov: HTTP 403 from SBIR\.gov/),
    ).toBeInTheDocument();
  ...
  it("marks a grants.gov set-aside as inferred and an SBIR.gov one as stated", async () => {
    ...
    expect(
      await screen.findByText("SBIR (inferred from title)"),
    ).toBeInTheDocument();
  ...
  it("ranks by focus and marks the scores as unvalidated model output", async () => {
    ...
    await user.type(
      screen.getByLabelText(/Research focus/),
      "GLP-1 co-agonists",
    );
    await user.click(
      screen.getByRole("button", { name: "Search and rank by focus" }),
    );
  ...
  it("reports a failed search instead of showing stale or invented results", async () => {
    const user = userEvent.setup();
    searchGrants.mockRejectedValue(
      new ApiError("grants request failed: timeout", 502),
    );
    render(<OpportunityFinder />);
  ...
  it("says so when the providers return nothing", async () => {
    ...
    expect(
      await screen.findByText("No opportunities matched those filters"),
    ).toBeInTheDocument();
  });
});
