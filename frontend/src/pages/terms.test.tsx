...
beforeEach(() => {
  ...
  registerCall.mockResolvedValue({
    ...
    user: {
      ...
      terms_accepted_at: "2026-09-08T00:00:01+00:00",
    },
  });
});
...
describe("the terms of agreement page", () => {
  it("shows the published version it is offering, without a session", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Terms of Agreement" }),
    ).toBeInTheDocument();
  ...
  it("states the acceptable use the assistant enforces in code", async () => {
    ...
    expect(
      await screen.findByText(/biological or chemical weapon/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/controlled substances or explosives/i),
    ).toBeInTheDocument();
  });

  // The provider refuses some phrasings of a legitimate question, and the page has to own that
  // rather than promise an answer it cannot deliver.
  it("says the model provider declines some requests of its own", async () => {
    render(
      <MemoryRouter>
        <TermsPage />
      </MemoryRouter>,
    ...
    expect(
      await screen.findByText(/model provider applies its own safety policy/i),
    ).toBeInTheDocument();
  });
  ...
  it("renders its wording even when the version cannot be read", async () => {
    ...
    expect(
      (await screen.findAllByText(/not a medical device/i)).length,
    ).toBeGreaterThan(0);
...
describe("accepting the terms to register", () => {
  ...
  it("asks again after leaving the register form and coming back", async () => {
    ...
    await user.click(screen.getByRole("checkbox"));
    await user.click(
      screen.getByRole("button", { name: /Already have a workspace/ }),
    );
    await user.click(screen.getByRole("button", { name: /Create one/ }));
